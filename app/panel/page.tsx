...
export default function PanelPage() {
  ...
  const genericDevices = Array.from({ length: 5 }).flatMap((_, i) => [
  ...
  const allDispositivos = [...dispositivos, ...genericDevices];
  ...
  const handleAssignDeviceToU = async (deviceId: string, startU: number) => {
    ...
    const device = allDispositivos.find(d => d.id === deviceId);
    if (!device) return;
                  ...
                  <div className="flex flex-col w-full h-full">
                    ...
                    <div className="flex-1 overflow-hidden w-full h-full cursor-grab active:cursor-grabbing relative">
                      <TransformComponent wrapperClass="!w-full !h-full" contentClass="!w-full !h-full flex items-center justify-center">
                        ...
                        >
                          <img src="/plano_hospital.webp?v=3" alt="Plano del Hospital" className="w-full max-w-[1200px] h-auto block pointer-events-none" />
