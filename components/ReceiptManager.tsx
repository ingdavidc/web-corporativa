...
import { collection, doc, setDoc, increment, serverTimestamp, runTransaction, getDocs, addDoc } from "firebase/firestore";
import { db } from "@/lib/firebase";
import { Plus, Trash2, FileText, Download, History, ArrowLeft, Send, Check } from "lucide-react";
...
export default function ReceiptManager() {
  ...
  const [clientInfo, setClientInfo] = useState({
    name: "", document: "", phone: "", address: "", city: ""
  });
  ...
  useEffect(() => {
    if (viewMode === 'history') {
      const fetchHistory = async () => {
        ...
        try {
          const qs = await getDocs(collection(db, 'recibos'));
          const list: any[] = [];
  ...
  const calculateTotal = (prods: Product[] = products) => {
    return prods.reduce((acc, curr) => acc + (curr.quantity * curr.unitPrice), 0);
  };
  ...
  const generatePDF = async (receiptNum: string, data: any) => {
    const doc = new jsPDF();
    ...
    // Si viene del historial, usar su fecha, sino la de hoy
    const dateStr = data.createdAt 
      ? new Date(data.createdAt.toMillis ? data.createdAt.toMillis() : data.createdAt).toLocaleDateString('es-CO', { year: 'numeric', month: 'long', day: 'numeric' })
    ...
    doc.setFont("helvetica", "normal");
    doc.setTextColor(grisPizarra[0], grisPizarra[1], grisPizarra[2]);
    doc.text(`Nombre/Razón Social: ${data.clientInfo.name}`, 20, 82);
    doc.text(`NIT/CC: ${data.clientInfo.document || ''}`, 20, 88);
    doc.text(`Teléfono: ${data.clientInfo.phone || ''}`, 20, 94);
    
    doc.text(`Dirección: ${data.clientInfo.address || ''}`, 105, 82);
    doc.text(`Ciudad: ${data.clientInfo.city || ''}`, 105, 88);

    const tableData = data.products.filter((p: any) => p.description.trim() !== "").map((p: any) => [
      p.description,
      p.quantity.toString(),
    ...
    autoTable(doc, {
      ...
      body: tableData,
      theme: 'grid',
      headStyles: { fillColor: azulCorporativo as [number, number, number], textColor: 255, fontStyle: 'bold', halign: 'center' },
      columnStyles: {
      ...
      styles: { fontSize: 10, textColor: grisPizarra as [number, number, number], lineColor: [220, 220, 220] },
      alternateRowStyles: { fillColor: grisPlatino as [number, number, number] }
    });
    ...
    doc.setFontSize(10);
    doc.setTextColor(azulCorporativo[0], azulCorporativo[1], azulCorporativo[2]);
    doc.text(`Método de Pago: ${data.paymentMethod || 'Efectivo'}`, 15, finalY + 5);
    
    if (data.notes) {
      doc.setFont("helvetica", "normal");
      doc.setFontSize(9);
      doc.setTextColor(grisPizarra[0], grisPizarra[1], grisPizarra[2]);
      const splitNotes = doc.splitTextToSize(`Notas: ${data.notes}`, 100);
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8);
    doc.text("Este documento es un comprobante de pago generado electrónicamente.", 105, finalY + 45, { align: "center" });
    doc.text("Válido sin firma ni sello.", 105, finalY + 49, { align: "center" });

    const securityCode = Math.random().toString(36).substring(2, 12).toUpperCase();
    doc.setFont("helvetica", "bold");
    doc.setTextColor(azulCorporativo[0], azulCorporativo[1], azulCorporativo[2]);
    doc.text(`CÓDIGO DE SEGURIDAD: ${securityCode}`, 105, finalY + 55, { align: "center" });
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      await generatePDF(receiptNum, payload);
      ...
      alert(`✅ Recibo ${receiptNum} generado exitosamente y formulario reiniciado.`);
    } catch (error) {
  ...
  const sendWhatsApp = (receipt: any) => {
    if (!receipt.clientInfo?.phone) {
      alert("El cliente no tiene un teléfono registrado para enviar WhatsApp.");
      return;
    }
    const phone = receipt.clientInfo.phone.replace(/[^0-9]/g, '');
    const text = `Hola ${receipt.clientInfo.name}, adjuntamos el comprobante de pago ${receipt.receiptNumber} por valor de ${formatCurrency(receipt.total)}. DC Telemática.`;
    window.open(`https://wa.me/57${phone}?text=${encodeURIComponent(text)}`, '_blank'); // Añadido prefijo de Colombia (57) asumiendo por la moneda
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 pb-4 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm">Crea y administra recibos de pago en PDF.</p>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          <button 
            ...
            className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${viewMode === 'form' ? 'bg-[#06b6d4] text-black' : 'bg-white/5 text-gray-400 hover:bg-white/10 border border-white/10'}`}
          >
            Nuevo Recibo
          </button>
          <button 
            onClick={() => setViewMode('history')}
            className={`px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all ${viewMode === 'history' ? 'bg-[#222A68] text-white border border-[#222A68]' : 'bg-white/5 text-gray-400 hover:bg-white/10 border border-white/10'}`}
          >
      ...
      {viewMode === 'history' ? (
        <div className="space-y-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-xl font-bold text-[#06b6d4]">Recibos Generados</h3>
            <button 
              onClick={() => {
                setIsLoadingHistory(true);
                getDocs(collection(db, 'recibos')).then(qs => {
                  const list: any[] = [];
                  qs.forEach(doc => list.push({ id: doc.id, ...doc.data() }));
                  list.sort((a,b) => (b.createdAt?.toMillis() || 0) - (a.createdAt?.toMillis() || 0));
                  setHistory(list);
              ...
              className="text-sm text-[#06b6d4] hover:text-white flex items-center gap-1 transition-colors"
            >
              Refrescar
            </button>
          </div>
          ...
          {isLoadingHistory ? (
            ...
            <p className="text-gray-500 text-center py-10">No hay recibos generados aún.</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm text-gray-300">
                <thead className="bg-white/5 text-gray-400 font-bold">
                  <tr>
                    <th className="p-3 rounded-tl-lg">Número</th>
                ...
                <tbody className="divide-y divide-white/5">
                  {history.map((h, i) => (
                    <tr key={h.id || i} className="hover:bg-white/5 transition-colors">
                      <td className="p-3 font-medium text-[#06b6d4]">{h.receiptNumber}</td>
                      <td className="p-3">{h.clientInfo?.name}</td>
                      <td className="p-3">
                        {h.createdAt ? new Date(h.createdAt.toMillis ? h.createdAt.toMillis() : h.createdAt).toLocaleDateString('es-CO') : 'N/A'}
                      </td>
                      <td className="p-3 text-right font-bold text-white">{formatCurrency(h.total)}</td>
                      <td className="p-3 flex justify-center gap-2">
                        <button 
                          onClick={() => generatePDF(h.receiptNumber, h)}
                          className="bg-[#222A68]/40 hover:bg-[#222A68] text-white p-2 rounded-lg transition-colors border border-[#222A68]/50"
                        ...
                        <button 
                          ...
                          className="bg-green-600/20 hover:bg-green-600 text-green-400 hover:text-white p-2 rounded-lg transition-colors border border-green-600/30"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Nueva Sección: Notas y Método de Pago */}
            <div className="bg-white/5 border border-white/10 rounded-xl p-5 grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <select
                  ...
                  className="w-full bg-black/40 border border-white/10 rounded-lg p-3 text-white focus:border-[#06b6d4] outline-none transition-colors"
                >
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-300 mb-2">Notas / Observaciones (Opcional)</label>
                <textarea
                  name="notes"
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  rows={2}
                  className="w-full bg-black/40 border border-white/10 rounded-lg p-3 text-white focus:border-[#06b6d4] outline-none transition-colors resize-none text-sm"
            ...
            <div className="bg-[#222A68]/10 border border-[#222A68]/30 rounded-xl p-6 sticky top-6">
              ...
              >
                {isGenerating ? (
                  ...
                  <>
                    <Download size={20} /> Generar y Guardar
                  </>
                )}
              </button>
              <p className="text-center text-xs text-gray-500 mt-4 flex items-center justify-center gap-1">
                <Check size={12} className="text-green-500" /> El formulario se limpiará tras guardar.
              </p>
            </div>
          </div>
        </div>
